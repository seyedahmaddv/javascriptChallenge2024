...
function checkDuplicate() {
    const itemText = itemInput.value.trim().replace(/\s+/g, ' '); // حذف فاصله اضافی

    // چک کردن تکراری بودن آیتم (حساسیت به بزرگ/کوچک بودن)
    const isDuplicate = listArr.some((item) => item.toLowerCase() === itemText.toLowerCase());

    if (!isDuplicate && itemText !== '') { 
        listArr.push(itemText); // افزودن آیتم در صورت نبود تکراری
        renderList();
    } else if (itemText === '') {
        alert('Please enter a valid item!'); // هشدار برای ورودی خالی
    } else {
        alert('This item already exists in the list!'); // هشدار برای آیتم تکراری
    }
}
